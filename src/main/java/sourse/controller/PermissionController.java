...
public class PermissionController {
    ...
    @PatchMapping("/{name}")
    ApiResponse<PermissionResponse> update (@PathVariable String name, @RequestBody  @Valid PermissionUpdateRequest request){
        ApiResponse<PermissionResponse> apiResponse = new ApiResponse<>();
        apiResponse.setData(permissionService.update(name, request));
        return  apiResponse;
    }
    @DeleteMapping("/{name}")
